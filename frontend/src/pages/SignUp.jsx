    ...
    <div className='h-screen w-full flex align-middle justify-center bg-slate-100'>
      <div className='flex flex-col justify-center'>
        <div className='py-4 px-8 flex flex-col align-middle justify-center rounded-lg w-96 bg-white shadow-lg'>
          <p className='text-4xl font-bold text-center mb-6'>Sign Up</p>
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <InputBox 
              ...
              formik={formik}
            />
            <Button label={"Sign Up"} onSubmit={formik.handleSubmit} />
          </form>
          <BottomWarning label={"Already have an account?"} buttonText={"Login"} to={"/signin"} />
          {errorMessage && (
              <div className='flex justify-center align-middle'>
                <p className='text-red-400 text-md pr-3'>{errorMessage.message}</p>
                <p className='text-lg cursor-pointer' onClick={(e) => setErrorMessage(null)}>X</p>
              </div>
          )}
        </div>
      </div>
